...
public import Cslib.Computability.Machines.RoseTreeMachine.V3.Tools
...
lemma bitape_write_computes
    ...
    (h_v : PB.computes_enc env p_v v) :
    PB.computes_enc env (bitape_write p_t p_v) (t.write v) := by
  simp only [PB.computes_enc, encode_biTape, DataEncode_pair] at h_t h_v ⊢
  apply PB.cons_computes h_v (PB.tail_computes h_t)
...
lemma stackTape_cons_computes
    ...
    (stackTape_cons p_x p_st).computes_enc env (st.cons x) := by
  cases x with
  | none =>
...
lemma to_pair_computes {α β : Type} [DataEncode α] [DataEncode β]
    ...
    {a : α} {b : β}
    (h_a : p_a.computes_enc env a)
    (h_b : p_b.computes_enc env b) :
    (to_pair p_a p_b).computes_enc env (a, b) := by
  simpa [DataEncode.encode, to_pair] using
    PB.cons_computes h_a (PB.cons_computes h_b PB.empty_computes)
...
lemma bitape_move_computes {env : List Data} {p_t p_dir : PB} {t : BiTape Symbol} {d : Dir}
    (h_t : PB.computes_enc env p_t t)
    (h_dir : PB.computes_enc env p_dir d) :
    (bitape_move p_t p_dir).computes_enc env (t.move d) := by
  unfold PB.computes_enc bitape_move
  refine PB.ifEq_computes h_dir constant_computes ?_ ?_
  · intro hd_eq
...
lemma bitape_optionMove_computes {env : List Data} {p_t p_dir : PB}
    ...
    (h_t : PB.computes_enc env p_t t)
    (h_dir : PB.computes_enc env p_dir d) :
    (bitape_optionMove p_t p_dir).computes_enc env (t.optionMove d) := by
  unfold PB.computes_enc bitape_optionMove BiTape.optionMove
  match d with
