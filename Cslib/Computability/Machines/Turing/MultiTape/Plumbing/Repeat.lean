...
private lemma runFrom_round_restart (w w' : Fin k → List Bool) (out : List Bool) (u : ℕ)
  ...
  rw [runFrom_succ_eq_step', runFrom_left_wordsCfg w out u hact, hrun_u]
...
private lemma visited_subset_Icc (cfg : Cfg k Bool State₀ input) (u : ℕ) (l : Fin k) (s0 : ℕ)
  ...
  obtain ⟨lo, hi, hlo, hhi, hvisited⟩ := tm.exists_visitedByTapeHead_eq_Icc cfg u l
  rw [h0] at hlo hhi
  have hcard : (tm.visitedByTapeHead cfg u l).card ≤ s0 :=
    le_trans (tm.spaceUsedByTape_le_spaceUsed cfg u l) hsp
  rw [hvisited, Int.card_Icc] at hcard
  intro z hz
  rw [hvisited] at hz
...
public theorem exists_transformsTapes_repeat {J : Type*} {k : ℕ} (i : Fin k) (x : Bool)
  ...
  have hrec : ∀ n, n ≤ N j → ∃ (T : ℕ) (wsn : Fin k → List Bool),
    ...
    | zero =>
      intro _
      refine ⟨0, ws, by rw [runFrom_zero, hstart], hP0, by omega, fun m hm l => ?_⟩
      have : m = 0 := by omega
      subst this
      rw [runFrom_zero, hstart]
      simp only [Finset.mem_Icc, wordsCfg_workTapePos]
      omega
    | succ n ih =>
      intro hn
      obtain ⟨T, wsn, hrun, hPn, hT, hIcc⟩ := ih (by omega)
      obtain ⟨τ, hτ, wsn', hrunTm, hQ, hsp⟩ := hround j n (by omega) input wsn out hPn
      obtain ⟨u, hu, huhalt, huactive⟩ := exists_minimal_halting_time tm
        (wordsCfg input (some tm.q₀) wsn out) τ (by rw [hrunTm]; rfl)
  ...
  obtain ⟨τ, hτ, ws', hrunTm, hQ, hsp⟩ := hstop j input wsN out hPN
  obtain ⟨u, hu, huhalt, huactive⟩ := exists_minimal_halting_time tm
    (wordsCfg input (some tm.q₀) wsN out) τ (by rw [hrunTm]; rfl)
  have hu_run : tm.runFrom (wordsCfg input (some tm.q₀) wsN out) u =
      wordsCfg input none ws' out := by
  ...
  refine ⟨T + (u + 1), ?_, ws', ?_, hQ.1, ?_⟩
  · -- time bound
    have hexp : (N j + 1) * (t j + 1) = N j * (t j + 1) + (t j + 1) := by rw [add_mul, one_mul]
    have : u ≤ t j := le_trans hu hτ
    omega
