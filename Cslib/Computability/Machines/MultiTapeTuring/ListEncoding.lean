...
-- TODO use an type class that contains explicit "one" and "two" symbols instead.
...
/-- Evaluate a TM on a set of "tapes" containing lists of words using the encoding
`listToTape`. This returns `Part.none` also if the result is not a valid encoding of a list. -/
