...
public def encodesTape {k : ℕ} (params : EncodingParams k) (tapes : Fin k → BiTape Symbol) : Prop :=
  ∀ idx,
    ...
    ∀ i, (tapes idx).atPos i ≠ none → params.encodesCell idx i
...
instance (k : ℕ) : StrEnc (EncodedTapes k (Symbol := Symbol)) where
  ...
  fromData _ := none
  fromData_toData := by sorry

/-- The `toData` map for `EncodedTapes` is injective. -/
lemma EncodedTapes.toData_injective {k : ℕ} :
    Function.Injective
      (fun enc : EncodedTapes k (Symbol := Symbol) =>
        StrEnc.toData (encodeTapes enc.tapes enc.params)) := by
  intro x y hxy
  have henc : encodeTapes x.tapes x.params = encodeTapes y.tapes y.params :=
    StrEnc.toData_injective _ hxy
  -- Lengths agree.
  have hlen : x.params.length = y.params.length := by
    have := congrArg List.length henc
    simpa [encodeTapes] using this
  -- Cell-by-cell equality via the encoded list.
  have hcell : ∀ (p : ℕ) (hpx : p < x.params.length) (i : Fin k),
      (x.tapes i).atPos (x.params.start i + p) = (y.tapes i).atPos (y.params.start i + p)
      ∧ ((x.params.start i + (p : ℤ)) == 0) = ((y.params.start i + (p : ℤ)) == 0) := by
    intro p hpx i
    have hpy : p < y.params.length := hlen ▸ hpx
    have hpx' : p < (encodeTapes x.tapes x.params).length := by simpa [encodeTapes] using hpx
    have hpy' : p < (encodeTapes y.tapes y.params).length := by simpa [encodeTapes] using hpy
    have hget : (encodeTapes x.tapes x.params)[p]'hpx' =
                (encodeTapes y.tapes y.params)[p]'hpy' := by
      congr 1
    have happ : ((encodeTapes x.tapes x.params)[p]'hpx') i =
                ((encodeTapes y.tapes y.params)[p]'hpy') i := congrArg (· i) hget
    grind [encodeTapes]
  -- Extract `start` equality from the unique head-marked position.
  have hstart : x.params.start = y.params.start := by
    funext i
    obtain ⟨hx_head, _⟩ := x.h_vallid i
    set sx := x.params.start i with hsx
    set sy := y.params.start i with hsy
    have hx_le : sx ≤ 0 := hx_head.1
    have hx_lt : (0 : ℤ) - sx < x.params.length := hx_head.2
    set p : ℕ := (-sx).toNat with hp_def
    have hnn : 0 ≤ -sx := by omega
    have hp_eq : (p : ℤ) = -sx := by
      simp [hp_def, Int.toNat_of_nonneg hnn]
    grind
  -- Both `params` are equal.
  have hparams : x.params = y.params := by
    have := EncodingParams.mk.injEq (k := k) x.params.start x.params.length
      y.params.start y.params.length
    cases hp1 : x.params with
    | mk xs xl =>
    cases hp2 : y.params with
    | mk ys yl =>
    grind
  -- Tapes agree on every position.
  have htapes : x.tapes = y.tapes := by
    funext i
    apply BiTape.ext_atPos
    intro j
    obtain ⟨_, hx_all⟩ := x.h_vallid i
    obtain ⟨_, hy_all⟩ := y.h_vallid i
    by_cases hx_in : x.params.encodesCell i j
    · have hx_le : x.params.start i ≤ j := hx_in.1
      have hx_lt : j - x.params.start i < x.params.length := hx_in.2
      set p : ℕ := (j - x.params.start i).toNat with hp_def
      have hnn : 0 ≤ j - x.params.start i := by omega
      have hp_eq : (p : ℤ) = j - x.params.start i := by
        simp [hp_def, Int.toNat_of_nonneg hnn]
      grind
    · grind
  cases x with | mk xt xp xv =>
  cases y with | mk yt yp yv =>
  cases htapes; cases hparams; rfl

-- TODO continue here: Do we actually need an explicit fromData?
-- Is it enough to prove that toData is injective?
...
lemma geatHeadSymbol.semantics {k k' : ℕ} (tapeIdx : Fin k') (tapes out aux : Fin k)
  ...
  rw [h_copySymbol (encodeTapes encodedTapes.tapes encodedTapes.params)[idx]
        ...
        (by simp [Function.update, h_outl, h_tapes_out, h_tapes_out.symm])]
