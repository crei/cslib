...
lemma var_sem_val {i : ℕ} {x : α} (h : (var i : Routine α).valid env)
...
lemma var_valid_add {x : α} (xs : List Value) (k : ℕ)
...
`encode x`. -/
lemma var_sem_val_add {x : α} (xs : List Value) (k : ℕ)
    (hk : xs[k]? = some (.data (DataEncode.encode x)))
...
only on the values of its arguments (it may capture `env`, but not the loop bookkeeping). -/
...
lemma WhileSem.det {body : Routine (α × β) → Routine (α × β)} {p r r' : α × β}
    (h : WhileSem env body p r) (h' : WhileSem env body p r') : r = r' := by
...
lemma while_sem_val (i : Routine (α × β)) (body : Routine (α × β) → Routine (α × β))
