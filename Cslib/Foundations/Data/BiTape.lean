...
@[simp, scoped grind =]
lemma mk₁_nil : mk₁ ([] : List Symbol) = nil := rfl
...
lemma ext_get (t₁ t₂ : BiTape Symbol) (h_get_eq : ∀ p, t₁.get p = t₂.get p) : t₁ = t₂ := by
  ...
  · apply StackTape.ext_get
    intro p
    sorry --simpa [get] using h_get_eq (p + 1)
...
lemma get_mk₁ (l : List Symbol) (p : ℤ) :
  ...
  match p with
  | Int.ofNat 0 => cases l <;> simp [mk₁, get, nil]
  | Int.ofNat (n + 1) =>
    cases l with
    | nil => simp [mk₁, get, nil]
    | cons h t =>
      simp only [mk₁, get, StackTape.mapSome]
      grind
  | Int.negSucc n => cases l <;> simp [mk₁, get, nil]
...
@[simp, scoped grind =]
lemma get_move_right (t : BiTape Symbol) (p : ℤ) : t.moveRight.get p = t.get (p + 1) := by
...
/-- Simplification lemma that connects `moveInt` and the function view via `get`. -/
@[simp, scoped grind =]
...
@[simp, scoped grind =]
lemma moveInt_head (t : BiTape Symbol) (Δ : ℤ) :
